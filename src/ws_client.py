#!/usr/bin/env python3
"""
WebSocket client for Bybit with graceful reconnect,
tick parsing, storage enqueue, metrics, and last_tick tracking.
"""
...
from src.parse import parse_public_trade_item            # ← REQUIRED
from src.storage import enqueue_tick                     # ← REQUIRED (store ticks)
...
def _on_signal(signum, frame):
    """Graceful exit for systemd or Ctrl+C."""
    global _shutdown
...
async def single_session(endpoint: str, topics: List[str]):
    ...
    LOG.info("Connecting to WS: %s", endpoint)
    ...
    async with aiohttp.ClientSession(timeout=timeout) as session:
        async with session.ws_connect(endpoint) as ws:
            ...
            async for msg in ws:
                if _shutdown:
                    LOG.info("Shutdown flag detected — closing WS session")
                    break

                # ---------------------------------------------------------
                # TEXT MESSAGE (main Bybit message type)
                # ---------------------------------------------------------
                if msg.type == aiohttp.WSMsgType.TEXT:
                    ...
                    try:
                        ...
                        LOG.debug("JSON decode error", exc_info=True)
                        payload = msg.data

                    # pretty print for debugging
                    print(pretty(payload))

                    # update last ticks (public trade messages)
                    try:
                        if isinstance(payload, dict) and payload.get("topic", "").startswith("publicTrade"):
                            for item in payload.get("data", []):
                                symbol = item.get("s")
                                ts = item.get("T") or item.get("ts")
                                if symbol and ts:
                                    update_last_tick(symbol, int(ts))
                    except Exception:
                        LOG.debug("Error updating last_tick", exc_info=True)

                    # -----------------------------------------------------
                    # STORAGE WRITER: enqueue tick(s)
                    # -----------------------------------------------------
                    try:
                        if isinstance(payload, dict) and payload.get("topic", "").startswith("publicTrade"):
                            for item in payload.get("data", []):
                                parsed = parse_public_trade_item(item)
                                if parsed:
                                    await enqueue_tick(parsed)
                    except Exception:
                        LOG.exception("Error parsing/enqueueing tick")

                # ---------------------------------------------------------
                # BINARY MESSAGE
                # ---------------------------------------------------------
                elif msg.type == aiohttp.WSMsgType.BINARY:
                    LOG.debug("Binary message (%d bytes)", len(msg.data))
    ...
    LOG.info("WS session ended (clean exit)")
...
async def run_with_reconnect(endpoint: str = DEFAULT_WS):
    ...
    reconn = Reconnector(
        logger=LOG,
        base=1.0,       # initial backoff
        cap=60.0,       # max backoff time
        max_attempts=None
    )
...
async def main():
    ...
    try:
        ...
        metrics_increment("errors")
        LOG.exception("Fatal error in ws_client main()")
    finally:
...
if __name__ == "__main__":
    try:
        ...
        metrics_increment("errors")
        LOG.exception("Fatal top-level exception in ws_client")
